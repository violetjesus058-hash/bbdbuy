 ...
 * 2. Replace [Kakobuy Spreadsheet](docs.google.com) -> [Kakobuy Spreadsheet](https://kakobuylinki.com/)
...
function processArticle(filePath) {
  ...
  // Step 2: Check for existing Kakobuy Spreadsheet anchor (case-insensitive)
  const ksRegex = /\[(kakobuy spreadsheet)\]\(([^)]+)\)/gi;
  ...
  if (hasKsAnchor && !hasHomepageLink) {
    ...
    content = content.substring(0, first.index) +
      `[${first[1]}](https://kakobuylinki.com/)` +
  ...
  if (!hasKsAnchor) {
    ...
    const summaryNoLink = summary.replace(
      'visit the [Kakobuy Spreadsheet](https://kakobuylinki.com/) homepage',
      'visit the Kakobuy Spreadsheet homepage'
    );
    content = content.trimEnd() + '\n\n' + summaryNoLink + '\n';
  }
